...
function StartState:update(dt)
    ...
    if love.keyboard.wasPressed('down') then
        highlighted = highlighted == 1 and 2 or 3 
        gSounds['paddle-hit']:play()
    end
    if love.keyboard.wasPressed('up') then
        highlighted = highlighted == 3 and 2 or 1
        gSounds['paddle-hit']:play()
    end
...
function StartState:render()
    -- title
    love.graphics.setFont(gFonts['ark'])
    love.graphics.printf("Arkanoid", 0, VIRTUAL_HEIGHT / 3,
    ...
    love.graphics.printf("HIGH SCORES", 0, VIRTUAL_HEIGHT / 2 + 80,
        VIRTUAL_WIDTH, 'center')
    love.graphics.setColor(255, 255, 255, 255)
    if highlighted == 3 then
        love.graphics.setColor(103, 255, 255, 255)
    end
    love.graphics.printf("HELP", 0, VIRTUAL_HEIGHT / 2 + 100,
    VIRTUAL_WIDTH, 'center')    
    -- reset the color
    love.graphics.setColor(255, 255, 255, 255)
end
